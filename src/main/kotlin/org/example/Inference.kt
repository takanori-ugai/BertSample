...
fun main() {
    ...
    HuggingFaceTokenizer.newInstance("bert-base-uncased").use { tokenizer ->
        Model.newInstance(modelName).use { model ->
            ...
            val bertEncoder = BertBlock.builder()
            ...
            model.block = SequentialBlock().apply {
                add(bertEncoder)
            ...
            val translator = object : Translator<String, String> {
                override fun processInput(ctx: TranslatorContext, input: String): NDList {
                    ...
                    val typeIds = manager.zeros(ai.djl.ndarray.types.Shape(1, seqLen))
                    ...
                    val masks = manager.ones(ai.djl.ndarray.types.Shape(1, seqLen))
